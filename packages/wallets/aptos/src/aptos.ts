...
type SignMessageResponse = Awaited<
  ReturnType<AptosConnectWallet["signMessage"]>
>;
...
export type AptosAdapter = AptosConnectWallet;
...
export type SignedAptosTransaction = Awaited<
  ReturnType<AptosConnectWallet["signTransaction"]>
>;
export type AptosMessage = SignMessagePayload;
...
> {
  ...
  async connect(): Promise<string[]> {
    ...
    this.networkInfo = await this.adapter.getNetwork();
  ...
  isConnected(): boolean {
    return !!this.adapter.accounts?.length;
  }
  ...
  getAddress(): string | undefined {
    return this.adapter.accounts?.[0]?.address?.toString?.();
  }
