...
type NetworkInfo = Awaited<ReturnType<AptosConnectWallet["getNetwork"]>>;
// export type AptosAdapter = BaseWalletAdapter;
type AptosAdapter = AdapterPlugin;
...
type SignedAptosTransaction = Awaited<
  ReturnType<AptosConnectWallet["signTransaction"]>
>;
type AptosMessage = SignMessagePayload;
 ...
 */
const getOverride = (adapter: AptosAdapter): Partial<AptosAdapter> =>
...
const getPropertyByName = <T>(
  propertyName: keyof AptosAdapter,
  overrides: Partial<AptosAdapter>,
...
export class AptosWalletV4 extends Wallet<
...
> {
  ...
  private account?: AccountInfo | StandardAccountInfo;
  ...
  async connect(): Promise<string[]> {
    ...
    this.networkInfo = await this.adapter.network();
    // set accounts
    this.account = (await this.adapter?.account?.()) || undefined;
  ...
  async signTransaction(tx: any): Promise<SignedAptosTransaction> {
  ...
  async signAndSendTransaction(
    ...
    const result: any = await this.adapter?.signAndSubmitTransaction?.(tx);
  ...
  signMessage(message: AptosMessage): Promise<SignedAptosMessage> {
